...
const CategoryDsr = () => {
  ...
  const initialTableData = Array.from({ length: 10 }, (_, index) => ({
    companyItems: selectedType === "Beer(India)" ? `Beer ${index + 1}` : "", // Sample item for "Beer(India)"
    rate: "",
    amt: "",
    total: "",
    ...Array.from({ length: 31 }, (_, i) => ({ [`day${i + 1}`]: "" })).reduce(
      (acc, val, idx) => ({ ...acc, ...val }),
      {}
    ),
  }));
